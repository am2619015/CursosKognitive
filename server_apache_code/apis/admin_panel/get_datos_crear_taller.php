<?php 
	...
	if(!$conn){
		...
		try {
                $stm = $conn->prepare("SELECT c.id_curso,c.nombre FROM cursos c");
                $stm->execute();
		...
		if(strcmp($mensaje_de,"correcto") == 0){
				  ...
				  try{
					...
					$stm2 = $conn->prepare("SELECT id_cuestionario,COUNT(num_pregunta),SUBSTRING(pregunta_txt,1,60) FROM preguntas_cuestionarios GROUP BY id_cuestionario");
					$stm2->execute();

                    $rows2 = $stm2->fetchAll(PDO::FETCH_NUM);

					if($rows2>0){
                        $mensaje_de="correcto";
                        $dato_respuesta2 = json_encode($rows2);
                    }else{
                        $mensaje_de="incorrecto";
                        $dato_respuesta2 = "No hay estudiantes en la db que correspondan con lo indicado";
                    }
