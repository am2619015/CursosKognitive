<?php
    ...
    try{
        //$stm = $conn->prepare("SELECT * FROM users WHERE email=:correo");
            $stm = $conn->prepare("SELECT MAX(id_cuestionario+1) AS cuestionario_crear from preguntas_cuestionarios"); // funcion mysql con todos los datos
            ...
            $num_get = $rows[0][0];
                ...
                $dato_respuesta ="id cuestionario disponible ╚".$num_get;
                $mensaje_de="correcto";
