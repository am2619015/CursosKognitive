<?php
    ...
    try{
        ...
        //$stm = $conn->prepare("SELECT * FROM users WHERE email=:correo");
                ...
                if(strcmp($table_name,"no_ingresaron")==0){
                    $sql = "SELECT U.id,U.email FROM caducidad_cursos AS C LEFT JOIN users AS U ON C.id_usuario=U.id LEFT JOIN (SELECT DISTINCT(email_usuario) FROM logs_usuarios) AS RF ON RF.email_usuario = U.email WHERE RF.email_usuario IS NULL AND C.id_curso = :idc";
