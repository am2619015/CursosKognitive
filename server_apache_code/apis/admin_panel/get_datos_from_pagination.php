<?php
    ...
    $pag_actual=$data['user']["pagina_actual"];
    ...
    $pag_actual=BlockSQLInjection(strip_tags(trim($pag_actual)));
        ...
        switch($table_name){
            ...
            if($column_id_name != ''){
                if(strcmp($table_name,"users")==0){
                    $sql = "SELECT ".$obtencion." FROM ".$table_name." u WHERE rol='estudiante' ORDER BY ".$column_id_name." DESC LIMIT :indice,:numeroCargar";
                    $stm = $conn->prepare($sql); // funcion mysql con todos los datos
                }else if(strcmp($table_name,"caducidad_cursos")==0){
                    $sql = "SELECT ".$obtencion." FROM ".$table_name." c INNER JOIN users u ON u.id=c.id_usuario ".$condicion." id_curso DESC,id_usuario DESC LIMIT :indice,:numeroCargar";
                    $stm = $conn->prepare($sql); // funcion mysql con todos los datos
                }else if(strcmp($table_name,"notas")==0){
                    $sql = "SELECT ".$obtencion." FROM ".$table_name." n INNER JOIN users u ON n.id_estudiante=u.id INNER JOIN cursos c ON n.id_curso=c.id_curso INNER JOIN talleres t ON t.id_taller=n.id_taller ".$condicion." ".$column_id_name." DESC LIMIT :indice,:numeroCargar";
                    $stm = $conn->prepare($sql); // funcion mysql con todos los datos
                }else{
                    $sql = "SELECT ".$obtencion." FROM ".$table_name." u ".$condicion." ".$column_id_name." DESC LIMIT :indice,:numeroCargar";
                    $stm = $conn->prepare($sql); // funcion mysql con todos los datos
                }
                ...
                $p_actual_mysl = $pag_actual -1;
                $indice_inicio = $p_actual_mysl * $num_datos_por_pagina;
                $stm->bindValue(":indice", $indice_inicio,PDO::PARAM_INT);
