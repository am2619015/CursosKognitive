<?php
    ...
    $stm = $conn->prepare("SELECT (SELECT t.nombre_taller from talleres t WHERE t.id_taller=n.id_taller) AS taller, (SELECT c.nombre from cursos c WHERE c.id_curso=n.id_curso) AS curso, n.nota AS nota from notas n WHERE n.id_estudiante = (SELECT id from users WHERE email = :correo)"); // funcion mysql con todos los datos
